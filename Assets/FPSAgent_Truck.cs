...
public static class MyF_t {
    public static bool RandomBool () {
        return Random.Range (0, 2) == 0;
    }
}

public class FPSAgent_Truck : Agent {
    ...
    Vector3 nowAngles;
    Vector3 target_goal = Vector3.zero;
    ...
    public override void OnEpisodeBegin () {
        ...
        // target.localPosition = new Vector3 (Random.Range (-14, 15), -1.0f, 12.0f);
        target.localPosition = new Vector3 (0.0f, -1.0f, 12.0f);
        // if (MyF_v.RandomBool ()) {
        //     target.localPosition = new Vector3 (Random.Range (-14, 15), -1.0f, 12.0f);
        // } else {
    ...
    public override void OnActionReceived (float[] vectorAction) {
        ...
        Vector3 Target_move = target.localPosition;
        if((target_goal.x-0.01f <= target.localPosition.x && target.localPosition.x <= target_goal.x+0.01f) && (target_goal.y-0.01f <= target.localPosition.y && target.localPosition.y <= target_goal.y+0.01f)){
            targetmoveflag = false;
        }
        // if (target.localPosition == target_goal) {
        //     targetmoveflag = false;
        // }
        ...
        if (targetmoveflag) {
            ...
            if (target_goal.x > target.localPosition.x) {
                Target_move.x += 0.01f;
            } else if (target_goal.x < target.localPosition.x){
                Target_move.x -= 0.01f;
            }
            
            if (target_goal.y > target.localPosition.y) {
                Target_move.y += 0.01f;
            } else if (target_goal.y < target.localPosition.y){
                Target_move.y -= 0.01f;
            }

            target.localPosition = Target_move;

        } else {
            int posvalue = Random.Range (-13, 14);
            target_goal.x = (float) posvalue;
            posvalue = Random.Range (-1, 9);
            target_goal.y = (float) posvalue;
            target_goal.z = 12.0f;
            targetmoveflag = true;
        }
